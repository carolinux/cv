...
\normalsize{I am a Python developer wtih experience in data science and web development teams.
I believe in building things right, building the right things, and having fun while doing it. That means caring about code quality and testing, releasing small and iterating, practising open communication and always reflecting on my processes. Looking for a new position where I can make a difference.}
...
\begin{itemize}
\item[$\bullet$] Built support for new payment methods
\item[$\bullet$] Productionized older code by writing tests before refactoring
\item[$\bullet$] Improved the local setup (Docker/docker-compose) and integrated services into Circle CI
\end{itemize}
...
\item[$\bullet$] Extended existing functionality (product pages, CMS service and payment system) practising TDD and pair programming
...
\colorbox{shade}{\textcolor{text1}{
        \begin{tabular}{l|p{5.5cm}}
            \raisebox{-1.25cm}{\includegraphics[height=2.60cm]{photo_2018}} &
...
\begin{itemize}
\item[$\bullet$] Built a data pipeline based on celery to automatically process the raw data (various stages, eg: a MATLAB program developed by domain experts in sensor data, Jupyter notebooks and python modules, chained together)
...
\begin{itemize}
\item[$\bullet$] Created data pipelines to go from raw data to automatically updating web dashboards (using luigi) 
\item[$\bullet$] Created interactive visualizations of geospatial data (using PostGIS, d3, QGIS)
\item[$\bullet$] Built a library to unify access to all the different data sources from telco providers (in Scala)
\end{itemize}
...
& Very Large Databases Conference 2014\\
\textsc{2014}
& Speaker at PyData Berlin conference \\
